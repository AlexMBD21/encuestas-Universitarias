...
export default function NotificationsPanel({ open, onClose, notifications: notificationsProp, onVisibleCountChange }: Props) {
  ...
  return createPortal(
    <div
      ...
      ref={rootRef}
      className={`fixed right-0 z-[1000] w-full max-w-sm shadow-[-20px_0_80px_rgba(0,0,0,0.5)] border-l border-white/5 rounded-b-3xl flex flex-col ${cls}`}
      style={{ 
        top: topPx != null ? `${topPx}px` : '76px', 
        maxHeight: topPx != null ? `calc(100vh - ${topPx}px)` : 'calc(100vh - 76px)', 
        minHeight: 0, 
        backgroundColor: 'rgba(13, 20, 37, 0.98)',
    ...
    >
      <div className="flex flex-col h-full overflow-hidden">
          ...
          {!panelLoaded ? (
            ...
            {notifications.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 px-6 text-center">
                <div className="w-20 h-20 bg-white/5 rounded-3xl flex items-center justify-center mb-4 shadow-inner border border-white/5">
                  <span className="material-symbols-outlined text-slate-600 text-4xl">notifications_off</span>
                </div>
                <h4 className="font-black text-white text-sm mb-1 uppercase tracking-tight">Todo despejado</h4>
                <p className="text-[11px] text-slate-400 max-w-[200px] font-medium opacity-70">No tienes avisos pendientes por el momento.</p>
              </div>
            ) : (
              (() => {
        ...
        <div 
          onClick={onClose}
          className="px-6 py-6 bg-white/5 flex justify-center border-t border-white/5 cursor-pointer hover:bg-white/10 transition-all group/pill"
          title="Cerrar panel"
        >
            <div className="w-16 h-1 bg-white/10 rounded-full group-hover/pill:bg-blue-500 transition-all duration-300" />
        </div>
      </div>
    </div>,
    document.body
